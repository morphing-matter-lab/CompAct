...
from parameters import EPSILON, EUC_SPACE_DIM, SCREW_SPACE_DIM
from messages import MSG_UNIMPLEMENTED, MSG_INVALID_GOALS, MSG_VALID_GOALS, MSG_GOALS_DONE
...
from axialSubspace import AxialSubspace
...
class TranslationAct(Actuation):
    ...
    def __init__(self, info):
        ...
        startPt = np.asarray(info[2]).astype(np.float64)
        endPt = np.asarray(info[3]).astype(np.float64)
...
class DOFSystem(object):
    ...
    def _findBases(self):
        ...
        if(len(principleVecs) == 0): # rotation in all directions or none at all
            self.bases = np.eye(EUC_SPACE_DIM)
        else:
    ...
    def _findTranslations(self):
        ...
        axisTarget = np.zeros(EUC_SPACE_DIM)
        vecs = self.tVecs[:, :EUC_SPACE_DIM]
        permissibleTrans = []
    ...
    def _findRotations(self):
        ...
        trans = self.tVecs[:, EUC_SPACE_DIM:]
        permissibleRots = []
    ...
    def _findMotionSpaces(self):
        ...
        for freedom in self.freedoms:
            ...
            subspace = DOFSystem.outputTwists(subspace, self.tVecs, mode=motionType)
    ...
    def _validityCheck(self):

        if(self.isConstraint):
            self.isValid = np.linalg.norm(self.tVecs[..., :EUC_SPACE_DIM]) > EPSILON
    ...
    def solveTrans(self, axis):
        ...
        rotTarget = np.zeros(EUC_SPACE_DIM).reshape((-1, 1))
    ...
    def printInfo(self):
        ...
        axii = np.take(self.tVecs, range(EUC_SPACE_DIM), axis=-1)
        pts = np.take(self.tVecs, range(EUC_SPACE_DIM, SCREW_SPACE_DIM), axis=-1)
        print("")
        print("axii")
        print(axii)
        print("")
    ...
    def isMotionPermissible(self, motions):
        ...
        for motion in motions:
            ...
            vec = targetMotion[:EUC_SPACE_DIM]
    ...
    def isAxial(self):
        ...
        axial = self.tVecs[:,:EUC_SPACE_DIM]
    ...
    def findFreeRotations(self):
        ...
        vecs = self.orthoDOF()
        for vec in vecs:
            if(np.linalg.norm(vec[:EUC_SPACE_DIM]) > EPSILON):
                freeRots += [vec]
    ...
    def indiDOFs(self):
        ...
        for freedom in self.freedoms:
            ...
            if(motionType == 'r'):
                ...
                rotVec = [screwVec(axis, np.zeros(EUC_SPACE_DIM))]
    ...
    def normVecs(vecs):
        ...
        length = np.linalg.norm(vecs[:,:EUC_SPACE_DIM], axis=1).reshape((-1, 1))
    ...
    def checkGoalValidity(consSpace):
        ...
        isAxisless = np.linalg.norm(consSpace[:,:EUC_SPACE_DIM]) < EPSILON
    ...
    @staticmethod
    def outputTwists(subspace, tVecs, mode='r'):

        result = {}
        twist = np.matmul(tVecs.T, subspace.refPt)
        twist = normScrew(twist)
        axis, normal = twist[:EUC_SPACE_DIM], twist[EUC_SPACE_DIM:]
        ptOnAxis = fixFloat(np.cross(axis, normal))
        result["refPt"] = tuple(ptOnAxis)
        
        spanVecs = []
        if(subspace.spanRank > 0):
            # get all possible combincations of span vectors
            combs = []
            for i in range(subspace.spanRank):
                combs += list(combinations(subspace.spanSpace, i + 1))
